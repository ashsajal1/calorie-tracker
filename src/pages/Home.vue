...
<template>
  <div class="p-4 grid place-items-center">
    <h1 class="text-xl font-bold h-40 w-40 rounded-full p-6 bg-green-500 text-center grid place-items-center">
  ...
  <div class="p-3 flex justify-between items-center">
    ...
    <Button @click="visible = true" icon="pi pi-plus" severity="secondary" class="mt-4" />
  </div>

  <Dialog v-model:visible="visible" :header="`Add Food`" :modal="true" :draggable="false" :resizable="false">
    <div>
      <div class="flex flex-col gap-2">
        <InputText v-model="foodName" placeholder="Food Name" />
        <InputNumber v-model="calories" placeholder="Calories" />
        <Button class="w-full" @click="handleAddFood" icon="pi pi-plus" severity="success" />
      </div>
    </div>
  </Dialog>
  ...
  <div>
    <div class="flex flex-row w-full items-center gap-2 justify-between p-3 border-b dark:border-b-gray-700"
      v-for="food in history" :key="food.id">
